...
class cdCompany
{
	 ...
	 */
	function setParentCompany($a_val)
	{
	 ...
	 */
	function setContactFirstname($a_val)
	{
	 ...
	 */
	function setContactLastname($a_val)
	{
	 ...
	 */
	function setCompanyPassword($a_val)
	{
	 ...
	 */
	function setCreationUser($a_val)
	{
	 ...
	 */
	function setTitleAmendment($a_val)
	{
	 ...
	 */
	function setAddressAmendment($a_val)
	{
	...
	function read()
	{
		...
		$set = $ilDB->query("SELECT * FROM cd_company WHERE ".
	...
	function delete()
	{
		...
		$ilDB->manipulate("DELETE FROM cd_company WHERE "
			." id = ".$ilDB->quote($this->getId(), "integer")
			);
	 ...
	 * Get all companies
	 */
	static function getAllCompanies($a_incl_nr_users = false, $a_centers_limit = null,
		$a_filter = "")
	{
		global $ilDB;
	...
	protected static function lookupProperty($a_id, $a_prop)
	{
		...
		$set = $ilDB->query("SELECT $a_prop FROM cd_company WHERE ".
	 ...
	 * @return
	 */
	static function lookupCenterId($a_id)
	{
	...
	static function checkUniquePassword($a_pw, $a_id = 0)
	{
		...
		$set = $ilDB->query("SELECT id FROM cd_company WHERE ".
	 ...
	 * @return
	 */
	static function getCompanyIdForPassword($a_pw)
	{
		global $ilDB;
	 ...
	 */
	function exportExcel($a_pl, $a_users = null)
	{
		global $lng;
	 ...
	 * @return
	 */
	function getOverviewData($a_pl, $a_user_id)
	{
		global $lng;
		...
		foreach ($ses as $se)
		{
			$avg = ilSkillSelfEvaluation::getAverageLevel($se["id"], $a_user_id,
				$se["top_skill_id"]);
			if ($avg != null)
			{
	 ...
	 * @return
	 */
	static function getPossibleParentCompaniesForCenter($a_company_id)
	{
		global $ilDB;
		
		$center_id = cdCompany::lookupCenterId($a_company_id);
		
		$set = $ilDB->query($q = "SELECT * FROM cd_company ".
	 ...
	 */
	static function getChildCompanies($a_company_id)
	{
		global $ilDB;
		
		$set = $ilDB->query("SELECT * FROM cd_company ".
