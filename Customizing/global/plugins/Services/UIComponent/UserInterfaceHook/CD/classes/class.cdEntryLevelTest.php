...
class cdEntryLevelTest
{
	...
	function read()
	{
		...
		$set = $ilDB->query("SELECT * FROM cd_ext_test WHERE ".
 ...
 */
	...
	function delete()
	{
		...
		$ilDB->manipulate("DELETE FROM cd_ext_test WHERE "
			." id = ".$ilDB->quote($this->getId(), "integer")
			);
	}
	 ...
	 */
	static function getEntryLevelTestsOfUser($a_user_id, $a_one_per_lang = false)
	{
		global $ilDB;
	...
	protected static function lookupProperty($a_id, $a_prop)
	{
		...
		$set = $ilDB->query("SELECT $a_prop FROM cd_ext_test WHERE ".
	 ...
	 * @return
	 */
	static function lookupUserId($a_id)
	{
	 ...
	 * @return
	 */
	static function lookupLanguage($a_id)
	{
	 ...
	 * @return
	 */
	static function lookupCreationDate($a_id)
	{
	 ...
	 * @return
	 */
	static function writeTestSession()
	{
		global $ilUser, $ilDB;

		$ilDB->manipulate("DELETE FROM cd_ext_test_sess WHERE "
	 ...
	 */
	static function calcExtLevel($a_points)
	{
	 ...
	 * @return
	 */
	static function getLastTestDatesForCompany($a_company_id)
	{
		global $ilDB;
		
		$set = $ilDB->query("SELECT MAX(cd_ext_test.ts) ts, cd_ext_test.user_id FROM usr_data ".
