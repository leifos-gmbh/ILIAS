...
{
	static $lang_levels = array("A1", "A2", "B1", "B2", "C1", "C2");

	// never change IDs for a category!!!
	// only IDs are stored in the database
	static $talk_understand = array(
		"10" => "present_projects",
	 ...
	 * Get language levels
	 */
	static function getLanguageLevels()
	{
	 ...
	 */
	function setCourseLanguage($a_val)
	{
	 ...
	 */
	function setOtherLangSel($a_val)
	{
	 ...
	 */
	function setOtherLangFree($a_val)
	{
	 ...
	 */
	function setTalkUnderstanding($a_val)
	{
	 ...
	 */
	function setTechnicalLanguage($a_val)
	{
	 ...
	 */
	function setTechnicalLanguageFree($a_val)
	{
	 ...
	 */
	function getTechnicalLanguageFree()
	{
	...
	function read()
	{
		...
		$set = $ilDB->query("SELECT * FROM cd_needs_analysis WHERE ".
	...
	function update()
	{
		...
		$ilDB->manipulate($q = "UPDATE cd_needs_analysis SET ".
	...
	function delete()
	{
		...
		$ilDB->manipulate("DELETE FROM cd_needs_analysis WHERE "
			." id = ".$ilDB->quote($this->getId(), "integer")
			);
	 ...
	 */
	static function getNeedsAnalysesOfUser($a_user_id, $a_one_per_lang = false)
	{
		global $ilDB;

		$set = $ilDB->query("SELECT * FROM cd_needs_analysis ".
	...
	protected static function lookupProperty($a_id, $a_prop)
	{
		...
		$set = $ilDB->query("SELECT $a_prop FROM cd_needs_analysis WHERE ".
