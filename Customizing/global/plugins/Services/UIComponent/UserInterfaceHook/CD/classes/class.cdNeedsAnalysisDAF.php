...
class cdNeedsAnalysisDAF extends cdNeedsAnalysis
{
	 ...
	 */
	static function getQuestions($a_pl)
	{
		$q = array();
	...
	function read()
	{
		...
		$set = $ilDB->query("SELECT * FROM cd_needs_analysis_daf WHERE ".
	...
	function create()
	{
		...
		{
			$ilDB->insert("cd_needs_analysis_daf",
				array(
	...
	function update()
	{
		...
		foreach (self::$daf_questions as $k => $q)
		{
			$set = $ilDB->query("SELECT * FROM cd_needs_analysis_daf ".
				" WHERE na_id  = ".$ilDB->quote($this->getId(), "integer").
				" AND q_id  = ".$ilDB->quote($k, "integer")
			);
			if ($rec = $ilDB->fetchAssoc($set))
			{
				$ilDB->update("cd_needs_analysis_daf",
					array(
						"answer" => array("clob", $this->getDAFAnswer($k))
					),
					array(
	...
	static function getNeedsAnalysesOfUser($a_user_id, $a_one_per_lang = false)
	{
		...
		while ($rec = $ilDB->fetchAssoc($set))
		{
			$set2 = $ilDB->query("SELECT * FROM cd_needs_analysis_daf ".
