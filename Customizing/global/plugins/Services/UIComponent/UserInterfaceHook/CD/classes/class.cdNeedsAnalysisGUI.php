...
class cdNeedsAnalysisGUI
{
	 ...
	 * @return
	 */
	function listNeedsAnalyses()
	{
		global $tpl, $ilToolbar, $ilCtrl, $lng;

		$ilToolbar->addButton($this->pl->txt("add_needs_analysis"),
	...
	public function initNeedsAnalysisForm($a_mode = "edit")
	{
		...
		switch ($step)
		{
			case 0:
				...
				$cg = new ilCheckboxGroupInputGUI($this->pl->txt("other_languages_q"),
					"other_lang_sel");
				foreach (cdLang::getLanguages() as $k)
				{
				...
				$ta = new ilTextAreaInputGUI($this->pl->txt("other_lang_free_q"),
					"other_lang_free");
				//$ta->setCols();
				//$ta->setRows();
				//$ta->setInfo($lng->txt(""));
...
//				$tpl->addJavascript("./Customizing/global/plugins/Services/UIComponent/UserInterfaceHook/CD/js/lib/jquery-1.8.1.min.js");
		...
		else
		{
			if ($step > 0)
			{
				$this->form->addCommandButton("updateNeedsAnalysisBack",
					"< ".$this->pl->txt("previous_step"));
			}
			if ($step < 3)
			{
				$this->form->addCommandButton("updateNeedsAnalysis",
...
//			ilUtil::sendSuccess($lng->txt("msg_obj_modified"), true);
	 ...
	 */
	function updateNeedsAnalysisBack()
	{
	 ...
	 * Update needs analysis
	 */
	function updateNeedsAnalysis($a_back = false)
	{
		global $lng, $ilCtrl, $tpl;
	 ...
	 */
	function confirmNeedsAnalysisDeletion()
	{
		...
		else
		{
			...
			{
				$cgui->addItem("needs_analysis_id[]", $i,
...
//var_dump($na);
		...
		foreach (cdNeedsAnalysis::$talk_understand as $k => $l)
		{
			$ne = new ilNonEditableValueGUI($this->pl->txt($l),
				"talk_understand_".$k);
			if($ta[$k] === 0)
			{
