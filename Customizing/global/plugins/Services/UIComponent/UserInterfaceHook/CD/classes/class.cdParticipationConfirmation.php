...
class cdParticipationConfirmation
{
	...
	function update()
	{
		...
		$ilDB->manipulate("UPDATE cd_part_conf_temp SET ".
	...
	function delete()
	{
		...
		$ilDB->manipulate("DELETE FROM cd_part_conf_temp WHERE ".
	 ...
	 */
	static function addUploadedFile($a_file)
	{
	 ...
	 * @return
	 */
	static function getAllConfirmationFiles()
	{
		global $ilDB;
	...
	static function downloadConfirmationFile($a_pl, $a_ref_id, $a_conf, $a_placeholders, $date)
	{
		...
		// init rtf replacer
		include_once("./Services/Rtf/classes/class.ilRtfReplacer.php");
...
//		$parts = new ilParticipants(ilObject::_lookupObjId($a_ref_id));
//		$parts = $parts->getParticipants();
		...
		// deliver the zip
		ilUtil::deliverFile($tmpdir."/Teilnahme.zip", "Teilnahme.zip",
			"", false, false, false);
	 ...
	 * @return
	 */
	static function lookup($a_id, $a_field)
	{
		global $ilDB;
		
		$set = $ilDB->query("SELECT ".$a_field." FROM cd_part_conf_temp ".
