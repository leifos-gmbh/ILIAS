...
class ilExcAssMemberStateRepository
{
	...
	public function getSubmitableAssignmentIdsOfUser(array $exc_ids, int $user_id): array
	{
		...
		$set = $db->queryF('SELECT ass.id FROM exc_assignment ass LEFT JOIN exc_idl idl
	...
	public function getAssignmentIdsWithGradingNeeded(array $exc_ids)
	{
		...
		$set = $db->queryF('SELECT ass.id, COUNT(*) open_grading FROM exc_mem_ass_status st LEFT JOIN exc_assignment ass 
			ON (st.ass_id = ass.id)
			WHERE '.$db->in("ass.exc_id", $exc_ids, false, "integer").'
			AND st.status = %s
			AND st.returned = %s
