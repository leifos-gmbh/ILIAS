...
class ReadingTimeDBRepo
{
    ...
    {
        $db = $this->db;
        $set = $db->queryF("SELECT act_est_reading_time FROM content_object " .
            " WHERE id = %s ",
            ["integer"],
            [$lm_id]
        );
    ...
    {
        $db = $this->db;
        $set = $db->queryF("SELECT id, act_est_reading_time, est_reading_time FROM content_object " .
