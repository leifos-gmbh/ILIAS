...
class ilCDPermWrapper
{
	 ...
	 */
	static function getGlobalCDRoles()
	{
		return array(
				"CDC-SysAdmin" => "y",
	 ...
	 */
	static function getLocalCDRoles()
	{
		return array(
				"CDC-MA" => "y",
				"CDC-MA-Master" => "y",
	 ...
	 */
	static function isCDRole($a_role)
	{
	 ...
	 */
	static function getAdminCenters($a_mode = "")
	{
		...
		foreach ($roles as $r)
		{
			...
			if (!$all)
			{
				...
				foreach ($folds as $f)
				{
					if ($f == ROLE_FOLDER_ID)
					{
						if (in_array(strtolower($role_title),
	 ...
	 * @return
	 */
	static function initCompanyUserRoles($a_user_id, $a_company_id)
	{
...
//exit;
	 ...
	 * @return
	 */
	static function initEmployeeUserRoles($a_user_id, $a_center_id)
	{
		global $rbacadmin, $ilPluginAdmin, $rbacreview, $tree;
...
//		$rbacadmin->assignUser((int) 4, $a_user_id, true);
	 ...
	 * @return
	 */
	static function initTrainerUserRoles($a_user_id, $a_center_id)
	{
		global $rbacadmin, $ilPluginAdmin, $rbacreview, $tree;
	 ...
	 * @return
	 */
	static function initCenterUserRoles($a_user_id, $a_center_id)
	{
		global $rbacadmin, $ilPluginAdmin, $rbacreview, $tree;
