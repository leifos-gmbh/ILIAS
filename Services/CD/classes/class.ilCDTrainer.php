...
class ilCDTrainer
{
	...
	protected function read()
	{
		...
		// cd_trainer data
		$set = $ilDB->query("SELECT * FROM cd_trainer WHERE ".
		...
		$set = $ilDB->query("SELECT * FROM cd_trainer_mtl WHERE ".
			" id = ".$ilDB->quote($this->getId(), "integer")
			);
		...
		$set = $ilDB->query("SELECT * FROM cd_trainer_il WHERE ".
			" id = ".$ilDB->quote($this->getId(), "integer")
			);
		while ($rec = $ilDB->fetchAssoc($set))
		{
		...
		$set = $ilDB->query("SELECT * FROM cd_trainer_ls WHERE ".
			" id = ".$ilDB->quote($this->getId(), "integer")
			);
		while ($rec = $ilDB->fetchAssoc($set))
		{
		...
		$set = $ilDB->query("SELECT * FROM cd_trainer_fe WHERE ".
			" id = ".$ilDB->quote($this->getId(), "integer")
			);
		while ($rec = $ilDB->fetchAssoc($set))
		{
		...
		$set = $ilDB->query("SELECT * FROM cd_trainer_tl WHERE ".
			" id = ".$ilDB->quote($this->getId(), "integer")
			);
		while ($rec = $ilDB->fetchAssoc($set))
		{
		...
		$set = $ilDB->query("SELECT * FROM cd_trainer_tt WHERE ".
			" id = ".$ilDB->quote($this->getId(), "integer")
			);
		while ($rec = $ilDB->fetchAssoc($set))
		{
		...
		// exam licenses
		$set = $ilDB->query("SELECT * FROM cd_trainer_el WHERE ".
			" id = ".$ilDB->quote($this->getId(), "integer")
			);
		while ($rec = $ilDB->fetchAssoc($set))
		{
...
//echo "-".$this->cd_trainer[$f];
	...
	public function delete()
	{
		...
		foreach ($tables as $t)
		{
			$ilDB->manipulate("DELETE FROM ".$t." WHERE "
				." id = ".$ilDB->quote($this->getId(), "integer")
				);
		}
	}
...
//echo "-$q-";
		...
		{
			// cd_trainer_mtl data (mother tongue languages)
			$set2 = $ilDB->query("SELECT * FROM cd_trainer_mtl WHERE ".
			...
			$set2 = $ilDB->query("SELECT * FROM cd_trainer_il WHERE ".
				" id = ".$ilDB->quote($rec["usr_id"], "integer")
				);
			$il = array();
			$il_txt = array();
...
//				echo "<br>- ".$rec["tech_lang"]."-".$rec2["disc"]."-".$rec2["val"];
	...
	protected static function lookupProperty($a_id, $a_prop)
	{
		...
		$set = $ilDB->query("SELECT $a_prop FROM cd_trainer WHERE ".
...
//			unset($fields[$del_id]);
	...
	public static function isTrainer($a_user_id)
	{
		...
		$set = $ilDB->query("SELECT id FROM cd_trainer ".
