...
class ilCDTrainerGUI
{
	...
	*/
	 ...
	 * @return
	 */
	function setTrainerListTabs()
	{
		global $ilTabs, $ilCtrl, $lng;

		$ilCtrl->setParameter($this, "former", "");
		$ilTabs->addTab("current_trainer", $lng->txt("cd_current_trainer"),
			$ilCtrl->getLinkTarget($this, "listTrainers"));

		$ilCtrl->setParameter($this, "former", "1");
		$ilTabs->addTab("former_trainer", $lng->txt("cd_former_trainer"),
			$ilCtrl->getLinkTarget($this, "listTrainers"));
	 ...
	 */
	function confirmDeactivateTrainer()
	{
	 ...
	 */
	function confirmActivateTrainer($a_deactivate = false)
	{
		global $ilCtrl, $tpl, $lng;
	 ...
	 * Activate trainers
	 */
	function activateTrainer($a_deactivate = false)
	{
		global $ilCtrl, $lng;
		...
		if (is_array($_POST["tr_id"]))
		{
			foreach ($_POST["tr_id"] as $tr_id)
			{
				...
				if (ilObject::_lookupType($tr_id) == "usr")
				{
					...
					if (in_array(ilCDTrainer::lookupCenterId($tr_id),
						$user_admin_centers))
					{
	 ...
	 * Init trainer form
	 */
	function initTrainerForm($a_mode = "edit")
	{
		global $ilCtrl, $tpl, $lng, $ilTabs;

		if (!$this->user_is_trainer)
		{
			$ilTabs->setBackTarget($lng->txt("cancel")." & ".$lng->txt("back"),
				$ilCtrl->getLinkTarget($this, "listTrainers"));
		}
	 ...
	 * Confirm trainer (de)activation
	 */
	function confirmCurrentTrainer($a_current = true)
	{
		global $ilCtrl, $tpl, $lng;
	 ...
	 */
	function currentTrainer($a_current = true)
	{
		global $ilCtrl, $lng;
