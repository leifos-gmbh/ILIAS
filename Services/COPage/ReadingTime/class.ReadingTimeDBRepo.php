...
class ReadingTimeDBRepo
{
    ...
    {
        $db = $this->db;
        $set = $db->queryF("SELECT est_reading_time FROM page_object " .
    ...
    public function saveTime(
        ...
        string $parent_type,
        string $lang = "-",
        int $est_reading_time = 0) : void
    {
        $db = $this->db;
        $db->update("page_object", [
    ...
    public function getPagesWithMissingReadingTime(
        string $a_parent_type, int $a_parent_id)
    {
        $db = $this->db;
    ...
    public function getParentReadingTime(
        string $a_parent_type, int $a_parent_id) : int
    {
        $db = $this->db;
