...
class ilICalParser
{
    ...
    protected ?ilCalendarEntry $entry = null;
    ...
    public function setCalendarEntry(ilCalendarEntry $entry): void
    {
        $this->entry = $entry;
    }

    public function getCalendarEntry(): ?ilCalendarEntry
    {
        return $this->entry;
    }
    ...
    protected function writeEvent(): void
    {
        ...
        foreach ($this->getContainer()->getItemsByName('SUMMARY', false) as $item) {
            if (is_a($item, 'ilICalProperty')) {
                $this->entry->setTitle($this->purgeString($item->getValue()));
                break;
            }
        }
        // Search description
        foreach ($this->getContainer()->getItemsByName('DESCRIPTION', false) as $item) {
            if (is_a($item, 'ilICalProperty')) {
                $this->entry->setDescription($this->purgeString($item->getValue()));
                break;
            }
        }
        ...
        foreach ($this->getContainer()->getItemsByName('LOCATION', false) as $item) {
            if (is_a($item, 'ilICalProperty')) {
                $this->entry->setLocation($this->purgeString($item->getValue()));
                break;
            }
        }
        ...
        foreach ($this->getContainer()->getItemsByName('DTEND') as $end) {
            ...
            $this->entry->setEnd($end);
            $this->entry->setFullday($fullday);
        }

        if (!$this->entry->getStart() instanceof ilDateTime) {
        ...
        if (
            !$this->entry->getEnd() instanceof ilDateTime &&
            $this->entry->getStart() instanceof ilDateTime
        ) {
            $this->entry->setEnd($this->entry->getStart());
        }

        // save calendar event
        if ($this->category->getLocationType() == ilCalendarCategory::LTYPE_REMOTE) {
            $this->entry->setAutoGenerated(true);
        }
        if ($create) {
            $this->entry->save();
        } else {
