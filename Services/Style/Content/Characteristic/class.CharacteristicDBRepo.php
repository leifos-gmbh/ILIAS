...
class CharacteristicDBRepo
{
    ...
    ) : void
    {
        ...
        $db->manipulateF("DELETE FROM style_char_title " .
            " WHERE style_id = %s AND type = %s AND characteristic = %s ",
            ["integer", "text", "text"],
            [$style_id, $type, $characteristic]
        );
    ...
    public function replaceParameter(
        ...
        string $a_type,
        int $a_mq_id = 0,
        bool $a_custom = false) : void
    {
        $db = $this->db;

        $set = $db->queryF("SELECT * FROM style_parameter " .
