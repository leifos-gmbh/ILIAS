...
use ILIAS\Setup\Migration as ilSetupMigration;
...
class RecordFilesMigration implements ilSetupMigration
{
    ...
    protected function getFiles(): array
    {
        ...
        foreach (scandir($this->getExportDir()) as $file) {
            ...
            if (
                in_array($file, ['.', '..']) &&
                $file !== "export" &&
                !preg_match('/^export_([0-9])+$/', $file, $matches)
            ) {
                continue;
            }
        ...
        foreach ($dirs as $object_id => $dir) {
            ...
            foreach (ilFileUtils::getDir($dir) as $file_name => $file_data) {
                $files[$object_id][] = $dir . DIRECTORY_SEPARATOR . $file_name;
            }
        }
        return $files;
    }
