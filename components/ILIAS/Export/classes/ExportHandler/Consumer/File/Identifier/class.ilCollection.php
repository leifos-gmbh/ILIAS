<?php

/**
 * This file is part of ILIAS, a powerful learning management system
 * published by ILIAS open source e-Learning e.V.
 *
 * ILIAS is licensed with the GPL-3.0,
 * see https://www.gnu.org/licenses/gpl-3.0.en.html
 * You should have received a copy of said license along with the
 * source code, too.
 *
 * If this is not the case or you just want to try ILIAS, you'll find
 * us at:
 * https://www.ilias.de
 * https://github.com/ILIAS-eLearning
 *
 *********************************************************************/

declare(strict_types=1);

namespace ILIAS\Export\ExportHandler\Consumer\File\Identifier;

use ILIAS\Export\ExportHandler\I\Consumer\File\Identifier\ilCollectionInterface as ilExportHandlerConsumerFileIdentifierCollectionInterface;
use ILIAS\Export\ExportHandler\I\Consumer\File\Identifier\ilHandlerInterface as ilExportHandlerConsumerFileIdentifierInterface;

class ilCollection implements ilExportHandlerConsumerFileIdentifierCollectionInterface
{
    protected array $elements;
    protected int $index;

    public function __construct()
    {
        $this->elements = [];
        $this->index = 0;
    }

    public function withElement(
        ilExportHandlerConsumerFileIdentifierInterface $element
    ): ilExportHandlerConsumerFileIdentifierCollectionInterface {
        $clone = clone $this;
        $clone->elements[] = $element;
        return $clone;
    }

    public function toStringArray(): array
    {
        return array_map(
            function (ilExportHandlerConsumerFileIdentifierInterface $file_identifier) {
                return $file_identifier->getIdentifier();
            },
            $this->elements
        );
    }

    public function current(): ilExportHandlerConsumerFileIdentifierInterface
    {
        return $this->elements[$this->index];
    }

    public function key(): int
    {
        return $this->index;
    }

    public function next(): void
    {
        $this->index++;
    }

    public function rewind(): void
    {
        $this->index = 0;
    }

    public function valid(): bool
    {
        return isset($this->elements[$this->index]);
    }

    public function count(): int
    {
        return count($this->elements);
    }
}
