...
use ILIAS\Export\ExportHandler\I\Part\Manifest\ilHandlerInterface as ilExportHandlerPartManifestInterface;
use ilObject;
use ilXmlWriter;

class ilHandler implements ilExportHandlerPartManifestInterface
{
    ...
    public function getXML(bool $formatted = true): string
    {
        ...
        foreach ($this->export_info->getComponentInfos() as $component_info) {
            ...
            $path_without_export_dir_name = substr(
                $component_info->getPathInContainer(),
                strpos($component_info->getPathInContainer(), DIRECTORY_SEPARATOR) + 1
            );
